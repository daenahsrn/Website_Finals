<?php
session_start();
include 'CRUD Functions/conn.php';
?>
<!DOCTYPE html>
<html lang="en">
<head>
    <title>Backstreet Boys</title>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="website.css">
</head>

<body>
    <section class="header">
        <header id="header">
            <div class="navigators">
                <nav>
                    <a href="index.php" class="home_nav">Backstreet Boys</a>
                    <a href="about.html" class="about_nav">About</a>
                    <a href="topSongs.html" class="topsongs_nav">Top Hits</a>
                    <a href="history.html" class="history">Band's History</a>
                    <?php if (isset($_SESSION['username'])): ?>
                        <span style="color: #ffc107; margin-left: 20px; font-weight: bold;">Welcome, <?php echo htmlspecialchars($_SESSION['username']); ?>!</span>
                        <a href="CRUD%20Functions/logout.php" class="logout_nav" style="color: #ff6b6b; font-weight: bold;">Logout</a>
                    <?php else: ?>
                        <a href="CRUD%20Functions/login.php" class="login_nav" style="font-weight: bold;">Login</a>
                        <a href="CRUD%20Functions/register.php" class="register_nav" style="font-weight: bold;">Register</a>
                    <?php endif; ?>
                </nav>
            </div>
        </header>
    </section>

    <main class="contents_home">
        <section class="hero_banner">
            <img src="backstreetBoys.jpg" alt="Backstreet Boys Banner">
        </section>

        <section class="music_Home">
            <div class="music_container">
                <a href="https://www.youtube.com/watch?v=VVB6FI8QTFg&list=RDVVB6FI8QTFg&start_radio=1" class="IWantItThatWay" target="_blank" rel="noopener noreferrer">I Want It That Way <br></a>
                <a href="https://www.youtube.com/watch?v=0Gl2QnHNpkA&list=RD0Gl2QnHNpkA&start_radio=1" class="AsALongAsYouLoveMe" target="_blank" rel="noopener noreferrer">As Long As You Love Me <br></a>
                <a href="https://www.youtube.com/watch?v=6M6samPEMpM&list=RD6M6samPEMpM&start_radio=1" class="Everybody(BSBack)" target="_blank" rel="noopener noreferrer">Everybody (Backstreet's Back) <br></a>
            </div>
        </section>

        <section class="previews">
            <div class="preview-container">
                <div class="preview-card">
                    <h3>About the Band</h3>
                    <p>Discover the story behind Nick, Kevin, AJ, Howie, and Brian—the kings of pop who took the world by storm.</p>
                    <a href="about.html" class="preview-link">Learn More</a>
                </div>

                <div class="preview-card">
                    <h3>Preview their Top Releases</h3>
                    <p>Such as I want it that way, Everybody (Backstreet's Back), and more...</p>
                    <a href="topSongs.html" class="preview-link">Explore Songs</a>
                </div>
            </div>
        </section>
    </main>

    <section class="footer">
        <footer>
            <section id="socials" class="CallToAction">
                <a href="https://www.facebook.com/backstreetboys" class="facebook" target="_blank" rel="noopener noreferrer"><img src="facebook icon.jpg" alt="Facebook"></a>
                <a href="https://www.instagram.com/backstreetboys" class="instagram" target="_blank" rel="noopener noreferrer"><img src="instagram icon.jpg" alt="Instagram"></a>
                <a href="https://x.com/backstreetboys" class="twitter" target="_blank" rel="noopener noreferrer"><img src="twitter.jpg" alt="Twitter"></a>
                <a href="https://www.youtube.com/user/bsbofficial" class="youtube" target="_blank" rel="noopener noreferrer"><img src="yt icon.jpg" alt="YouTube"></a>
                <a href="https://open.spotify.com/artist/5rSXSAkZ67PYJSvpUpkOr7?nd=1&dlsi=ef4b4bf4085344ed" class="spotify" target="_blank" rel="noopener noreferrer"><img src="spotify icon.jpg" alt="Spotify"></a>
                <a href="https://www.tiktok.com/@backstreetboys" class="tiktok" target="_blank" rel="noopener noreferrer"><img src="tiktok icon.jpg" alt="TikTok"></a>
            </section>     
            <p><br>© 2026 Backstreet Boys | All Rights Reserved</p>
        </footer>
    </section>
</body>
</html>
